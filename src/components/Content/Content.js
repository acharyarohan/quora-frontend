...
export class Content extends Component {
	...
	userContentHandler() {
		...
		if (this.state.selectedTab === "Questions Asked") {
			axios
				.get(`http://localhost:3001/users/questionsAsked/${userId}`)
				.then(response => {
			...
			axios
				.get(`http://localhost:3001/users/${userId}/topics`)
				.then(response => {
					console.log("Status Code: ", response.status);
					if (response.status === 200) {
			...
			axios
				.get(`http://localhost:3001/users/${userId}/questions`)
				.then(response => {
					console.log("Status Code: ", response.status);
					if (response.status === 200) {
						...
						<div className="col-lg-2 col-md-2 col-xs-12 left-stick">
							<Row>
								...
								<div id="feeds_options">
									{this.state.settings_options.map(
										(option, index) => {
											return (
												<div
													onClick={() => {
							...
							<div>
								...
								"Questions Asked" ? (
									<div>
										...
										<List
											dataSource={
												this.state.questionsAskedPayload
													.results
											}
											renderItem={result => (
												<List.Item>
